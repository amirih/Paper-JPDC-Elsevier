...
int main(){
    ...
    __m256 dep1, dep2; //  dep = { 1,   3,  5, 7,  9,  11, 13, 15 }
    __m256 coeff = _mm256_set1_ps(8*c); //coeff = { 16, 16, 16, 16, 16, 16, 16, 16 }
    ...
    for(; i<LEN-1; i+=16){
        ...
        dep2 = _mm256_load_ps( &a[i]);
        dep2 = _mm256_add_ps(dep2, coeff);
        _mm256_store_ps(&a[i+8], dep2);  
